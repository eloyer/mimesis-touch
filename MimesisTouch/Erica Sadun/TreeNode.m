...
#pragma mark new additions by Erik Loyer

// Return the attribute value that matches the key
- (NSString *) attributeForKey: (NSString *) aKey {
	return [self.attributeDict objectForKey:aKey];
}

- (BOOL) hasAttribute:(NSString *)aKey {
	id object = [self.attributeDict objectForKey:aKey];
	BOOL result;
	if (object) {
		result = TRUE;
	} else {
...
// Return all children that match the key (no recursion).
...
- (void) dealloc
{
	...
	self.key = nil;
	self.leafvalue = nil;
	if (self.attributeDict) [self.attributeDict release];
